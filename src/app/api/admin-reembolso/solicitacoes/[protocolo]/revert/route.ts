...
export async function POST(
  ...
  try {
    ...
    // Revert to pendente and clear approval/rejection fields
    ...
    if (updateError) {
      return NextResponse.json(
        { error: 'Erro ao reverter solicitação', details: updateError.message },
        { status: 500 }
      );
    }
    ...
    await supabaseAdmin.from('log_acoes').insert({
      ...
      acao: 'revertida',
      detalhes: `Solicitação revertida para pendente por ${session.nome}`,
    });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Erro ao reverter solicitação:', error);
    return NextResponse.json(
